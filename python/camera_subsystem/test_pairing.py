...
class PairerTest(unittest.TestCase):
    def test_matched(self):
        p = Pairer(pair_window=4.0, flight_source=flight_source)
    ...
    def test_duplicate_within_loop_window(self):
        p = Pairer(pair_window=4.0, loop_window=200.0, flight_source=flight_source)
        p.submit_scan(ts=100.0, barcode="TAG1")
    ...
    def test_same_tag_after_loop_window_is_a_fresh_match(self):
        p = Pairer(pair_window=4.0, loop_window=50.0, flight_source=flight_source)
        p.submit_scan(ts=100.0, barcode="TAG1")
